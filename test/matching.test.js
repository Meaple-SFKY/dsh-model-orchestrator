...
test('pool ratings are relative, measured, and never name a model', async () => {
  const { ratePool } = await import('../lib/discovery.js')

  const weak = profile('p1', 'weak', { name: 'Weak', contextWindow: 8000 })
  const strong = profile('p1', 'strong', {
    name: 'Strong',
    contextWindow: 200000,
    efforts: ['high'],
  })
  const ratings = ratePool([weak, strong])

  // The strongest model available rates 5; the weakest rates 1.
  assert.equal(ratings['p1/strong'].stars, 5)
  assert.equal(ratings['p1/weak'].stars, 1)
  assert.ok(ratings['p1/strong'].stars > ratings['p1/weak'].stars)

  // A pool with nothing to compare against yields NO stars rather than awarding
  // an unbeatable-looking score to the only option.
  const alone = ratePool([weak])
  assert.equal(alone['p1/weak'].stars, null, 'a one-model pool has nothing to rank')
  assert.equal(alone['p1/weak'].comparable, false)

  // Every rating is in range, comparable, and free of any cost claim.
  for (const rating of Object.values(ratings)) {
    assert.ok(Number.isInteger(rating.stars) && rating.stars >= 1 && rating.stars <= 5)
    assert.equal(rating.comparable, true)
    assert.ok(!('cost' in rating) && !('costIndex' in rating), 'no cost is reported')
  }
})

test('a pool whose models are indistinguishable grades them equally', async () => {
  const { ratePool } = await import('../lib/discovery.js')
  const a = profile('p1', 'a', { name: 'A', contextWindow: 64000 })
  const b = profile('p1', 'b', { name: 'B', contextWindow: 64000 })
  const ratings = ratePool([a, b])
  assert.equal(ratings['p1/a'].stars, null, 'no spread means no ranking to show')
  assert.equal(ratings['p1/b'].stars, null)
  assert.equal(ratings['p1/a'].score, null)
})

test('a pool with nothing to compare reports no ranking, not a top score', async () => {
  const { ratePool } = await import('../lib/discovery.js')
  const only = profile('p1', 'only', { name: 'Only', contextWindow: 8000 })
  const rating = ratePool([only])['p1/only']
  assert.equal(rating.stars, null)
  assert.equal(rating.comparable, false)
  // Nothing about cost is reported: the panel shows no price.
  assert.ok(!('cost' in rating))
});

test('no model name, provider, or vendor takes part in rating', async () => {
  const { ratePool } = await import('../lib/discovery.js')
  // Two structurally identical models with different names must rate identically,
  // which proves the rating reads only measured facts.
  const left = profile('vendor-a', 'alpha', { name: 'Alpha', contextWindow: 128000, efforts: ['high'] })
  const right = profile('vendor-b', 'beta', { name: 'Beta', contextWindow: 128000, efforts: ['high'] })
  const ratings = ratePool([left, right])
  assert.deepEqual(
    { stars: ratings['vendor-a/alpha'].stars, cost: ratings['vendor-a/alpha'].cost },
    { stars: ratings['vendor-b/beta'].stars, cost: ratings['vendor-b/beta'].cost },
  )
});
