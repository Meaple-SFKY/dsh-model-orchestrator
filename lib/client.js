...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ensureBoardStyles() {
      ...
      style.textContent = `
.dshmo-board{display:flex;flex-direction:column;gap:12px;padding:14px;font-size:13px;color:var(--dsw-alias-label-primary)}
...
`
    ...
    /** `provider/model` split for the chip, so the model name can be emphasised. */
    function ModelChip({ route, t }) {
      if (typeof route !== 'string' || route === '') {
        return h('span', { className: 'dshmo-model' }, t('board.noRoute'))
      }
      const slash = route.indexOf('/')
    ...
    function DelegationNode({ row, t }) {
      ...
      const marker = raw.lastIndexOf(' via ')
      const capability = marker === -1 ? (raw === '' ? undefined : raw) : raw.slice(0, marker)
      const route = marker === -1 ? undefined : raw.slice(marker + 5)
      return h(
        'div',
        {
