...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      'pool.priceLine': '{input} / {output} USD per M tok',
    ...
    const zh = {
      ...
      'pool.priceLine': '{input} / {output} 美元每百万 token',
...
`
    ...
    function AssignmentCard({ state, onApply, t }) {
      ...
      const labelFor = (id) => keys.find((entry) => entry.key === id)?.label ?? id
              ...
              models.map((model) =>
                h(
                  ...
                  h(
                    'td',
                    { style: styles.td },
                    poolTierLabel(model.tier, t),
                    // Public facts, labelled as researched the moment they are shown.
                    model.researched === undefined
                      ...
                      : h(
                          ...
                          model.researched.matched === true
                            ? t('pool.researchedAs', {
                                name: model.researched.publicName ?? model.researched.route ?? '',
                              })
                            : t('pool.researchedUnconfirmed'),
                          model.researched.inputPerMTok === undefined &&
                          model.researched.outputPerMTok === undefined
                            ? null
                            : h(
                                'div',
                                { style: { ...styles.mono, fontSize: '11px' } },
                                t('pool.priceLine', {
                                  input: model.researched.inputPerMTok ?? '—',
                                  output: model.researched.outputPerMTok ?? '—',
                                }),
                              ),
                        ),
                  ),
