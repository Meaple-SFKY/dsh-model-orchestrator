...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Render a 1-5 star capability rating.
     *
     * The rating is RELATIVE to the current pool (see the host's `ratePool`), which
     * both the tooltip and the note under the table state. When the pool has no
     * measurable spread the host reports no stars, and this renders a dash rather
     * than awarding a top score to the only option.
     */
    function Stars({ rating, t }) {
      if (rating == null || rating.stars == null) {
        return h('span', { style: styles.muted, title: t('pool.starsFlat') }, t('pool.noValue'))
      }
      ...
      return h(
        'span',
        {
          ...
          title: t('pool.starsAria', { stars }),
          'aria-label': t('pool.starsAria', { stars }),
        },
        '\u2605'.repeat(stars),
        h('span', { style: { color: 'var(--dsw-alias-border-l2)' } }, '\u2606'.repeat(5 - stars)),
      )
    }
    ...
    function PoolTable({ pool, t }) {
      ...
      return h(
        ...
        h(
          ...
          h(
            'table',
            { style: { ...styles.table, minWidth: '760px' } },
            h(
              'thead',
              null,
              h(
                ...
                h('th', { style: styles.th }, t('pool.columnStars')),
                ...
                h('th', { style: styles.th }, t('pool.columnDeclared')),
            ...
            h(
              ...
              models.map((model) =>
                h(
                  ...
                  h('td', { style: styles.td }, h(Stars, { rating: model.rating, t })),
                  ...
                  h(
                    ...
                    (model.domains ?? []).length > 0
                      ...
                      : h(
                          'span',
                          { style: styles.muted, title: t('pool.declaredNoneHint') },
                          t('pool.declaredNone'),
                        ),
                  ),
                  h(
          ...
          h('p', { style: { ...styles.muted, marginTop: '8px' } }, t('pool.ratingNote')),
        ),
      )
    }
     ...
     * parts of the task were delegated, how they relate, and what is running right
     * now. Topology is harness-native (`/tree` reads the durable session tree), so
     * nothing here is a parallel task tracker.
     */
    function OrchestratorBoard(props) {
      ...
      /** One graph row: connector prefix plus the node's facts. */
      const GraphRow = ({ row }) => {
        // Indentation plus a vertical rule per ancestor level, so the shape reads
        // as a tree without needing a canvas.
        const prefix = row.parentIsLast
          .map((isLast) => (isLast ? '   ' : '│  '))
          .join('')
        const live = row.activity === 'running'
        return h(
          'div',
          { style: { ...styles.row, gap: '6px', alignItems: 'baseline', padding: '2px 0' } },
          h(
            'span',
            { style: { ...styles.mono, color: 'var(--dsw-alias-border-l2)', whiteSpace: 'pre' } },
            `${prefix}${'   '.repeat(Math.max(0, row.depth - 1))}${row.depth > 0 ? '├─ ' : ''}`,
          ),
          h(
            'span',
            {
          ...
          h('span', { style: { fontWeight: live ? 600 : 400 } }, row.label ?? row.id.slice(0, 12)),
          h(
            'span',
            { style: styles.pill },
          ...
          h(
            'span',
            { style: { ...styles.mono, ...styles.muted } },
      ...
      const header = h(
        ...
        h(SectionTitle, {
          ...
          right: h(
            'div',
            { style: styles.row },
            h('span', { style: styles.pill }, t('board.statDelegated', { count: counts.total })),
            h('span', { style: styles.pill }, t('board.statRunning', { count: counts.running })),
            h('span', { style: styles.pill }, t('board.statRoots', { count: counts.roots })),
            h(
              'button',
              { type: 'button', style: styles.button, disabled: busy, onClick: () => void reload(true) },
              busy ? t('doc.refreshing') : t('doc.refresh'),
        ...
        h('p', { style: styles.muted }, t('board.description')),
      ...
      const graph = h(
        Card,
        null,
        h(SectionTitle, { title: t('board.graphTitle') }),
        sessionId === undefined
          ? h('p', { style: styles.muted }, t('board.noSession'))
          : rows.length === 0
            ? h('p', { style: styles.muted }, t('board.empty'))
            : h('div', { style: { marginTop: '8px' } }, rows.map((row) => h(GraphRow, { key: row.id, row }))),
        tree?.truncated === true
          ? h('p', { style: { ...styles.warn, marginTop: '6px' } }, t('board.truncated'))
          : null,
      )

      // Routing state stays reachable without leaving the board: the same
      // capacities the settings page reports, plus the live pool size.
      const capacity = h(
        Card,
        null,
        h(SectionTitle, { title: t('capacity.title') }),
        h(
          'div',
          { style: { ...styles.row, marginTop: '6px' } },
          h('span', { style: styles.pill }, t('board.mode', { mode: state?.mode ?? '—' })),
          h('span', { style: styles.pill }, t('strip.modelCount', { count: state?.pool?.size ?? 0 })),
          h('span', { style: styles.pill }, t('capacity.inFlight', { count: state?.inFlight ?? 0 })),
          h(
            'span',
            { style: styles.pill },
            t('capacity.capabilities', { count: (state?.capabilities ?? []).length }),
      ...
      return h('div', { style: { ...styles.root, padding: '12px' } }, header, graph, capacity)
    }
