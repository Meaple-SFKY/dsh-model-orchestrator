 ...
 *
 * Only *preferences*, *calibrations*, learned capability descriptors, and the
 * researched public facts about models persist. Two things are deliberately absent:
 *
...
import { normalizeResearchMap } from './model-research.js';
...
export function defaultState() {
  return {
    ...
    preferences: {
      ...
      reasoningEffort: {},
      // The standing division of labour: capability (or capability group) to an
      // ordered list of model targets. Targets are identities, not routes, so the
      // table survives the pool churning around it.
      capabilityAssignments: {},
      // Decision cues used ONLY when no calling model supplied an analysis. Empty
      // means "use the built-in fallback".
      decisionCues: {},
      // What a unit that declared a dependency receives from the units it depends
      // on: their complete answer (`full`, the default) or only its bounded preview
      // (`summary`). `maxChars` caps how much of that answer may enter the child's
      // prompt, and `includeStructured` decides whether a structured result travels
      // with it.
      handoff: { strategy: 'full', maxChars: 12_000, includeStructured: true },
      // Bounds on the two bounded collaborations a run allows. A sibling question is
      // a real model call and a review round re-runs paid work, so neither is
      // unbounded by default.
      questions: { maxPerRun: 4, timeoutMs: 240_000 },
      review: { maxRounds: 2 },
    },
    taxonomy: { custom: [] },
    profiles: {},
    // Public facts about models, researched on demand and keyed by identity. Kept
    // apart from `profiles` because it is external knowledge with provenance, not
    // a calibration this plugin measured.
    research: {},
    updatedAt: Date.now(),
  };
}
...
function normalizePreferences(input) {
  ...
  return {
    ...
    capabilityAssignments: normalizeAssignments(input.capabilityAssignments),
...
/** Validate the sibling-question budget. */
function normalizeQuestions(input) {
  const base = { maxPerRun: 4, timeoutMs: 240_000 };
  if (!isRecord(input)) return base;
  return {
    maxPerRun: Number.isSafeInteger(input.maxPerRun) ? clamp(input.maxPerRun, 0, 32) : base.maxPerRun,
    timeoutMs: Number.isSafeInteger(input.timeoutMs) ? clamp(input.timeoutMs, 1_000, 1_800_000) : base.timeoutMs,
  };
}

/** Validate the review-round budget. */
function normalizeReview(input) {
  const base = { maxRounds: 2 };
  if (!isRecord(input)) return base;
  return {
    maxRounds: Number.isSafeInteger(input.maxRounds) ? clamp(input.maxRounds, 0, 5) : base.maxRounds,
  };
}

/** Validate the dependency-handoff policy. */
function normalizeHandoff(input) {
  const base = { strategy: 'full', maxChars: 12_000, includeStructured: true };
  if (!isRecord(input)) return base;
  return {
    strategy: input.strategy === 'summary' ? 'summary' : 'full',
    maxChars: Number.isSafeInteger(input.maxChars) ? clamp(input.maxChars, 500, 200_000) : base.maxChars,
    includeStructured: input.includeStructured !== false,
  };
}
...
export function normalizeState(input) {
  ...
  return {
    ...
    research: normalizeResearchMap(input.research),
...
export class OrchestratorStore {
  ...
  #writeError;
  /**
   * Set when the file on disk was written by a NEWER plugin version.
   *
   * The error message alone did not protect it: `save()` wrote unconditionally and
   * cleared `#writeError`, and activation writes on its own (a pool refresh prunes
   * calibrations), so the first refresh replaced the very file the read had refused
   * to interpret. A downgrade must not destroy what it cannot read.
   */
  #refusedNewerSchema = false;
  ...
  #read() {
    try {
      ...
      if (schema !== undefined && schema > STATE_SCHEMA_VERSION) {
        ...
        // in-memory state falls back to defaults and the file is LEFT ALONE —
        // ` #refusedNewerSchema` is what makes that true for every later write.
        this.#refusedNewerSchema = true;
  ...
  save() {
    ...
    if (this.#refusedNewerSchema) {
      // Refuse to write at all: this file belongs to a newer plugin, and a write
      // here would silently downgrade it. Reported through `writeError`, which the
      // read already set, so the operator sees why nothing is being saved.
