...
export async function discoverModels(ctx, options = {}) {
  ...
  try {
    ...
    // THROW rather than report an empty pool. Returning `{ models: [] }` made a
    // failed registry read look like a successful discovery of nothing, so
    // `ModelPool.refresh` replaced a perfectly good pool with an empty one and left
    // `lastError` unset — the pool silently emptied instead of staying stale and
    // saying why. A throw lets the pool keep what it had and record the failure.
    throw new Error(`llm.listProviders() failed: ${String(error?.message ?? error)}`, {
      cause: error,
    });
  }
...
export class ModelPool {
  ...
  /**
   * What the registry advertised, before the deployment policy and the user's own
   * route preferences narrowed it.
   *
   * Kept so a change to EITHER can re-narrow the pool without a network round trip:
   * re-applying a filter is free, re-discovering is not.
   */
  #advertised = [];
  #advertisedProviders = [];
  #baseProblems = [];
  ...
  /**
   * Narrow the advertised set by the deployment policy and the user's preferences.
   *
   * Split out of `refresh` so both of those can change without a re-discovery: a route
   * preference is read from local state, and the deployment's subagent policy is a
   * local configuration read, so neither needs the provider listing that costs a
   * network round trip.
   */
  #applyFilter() {
    const filtered = applyRoutePolicy(this.#advertised, {
      policy: this.#policy,
    ...
    this.#problems = [
      ...
      ...(filtered.constrained && filtered.models.length === 0
        ? [
            `the deployment's subagent route policy matched none of the ${this.#advertised.length} advertised model(s), so nothing is routable`,
          ]
        : []),
    ];
  }
  ...
  /**
   * Re-apply the deployment policy and the user's route preferences to what was last
   * advertised, without asking the providers anything.
   *
   * @returns the number of routable models after filtering.
   */
  refilter() {
    if (this.#advertised.length === 0) return this.#models.length;
    this.#policy = { ...this.#policy };
    this.#applyFilter();
    return this.#models.length;
  }

  /** The provider ids the last discovery actually saw, before any filtering. */
  advertisedProviders() {
  ...
  async refresh(ctx, options = {}) {
    ...
    this.#refreshing = (async () => {
      try {
        ...
        this.#advertised = result.models;
        this.#advertisedProviders = uniqueStrings(
          (result.models ?? []).map((model) => model.provider),
        );
        this.#policy = readSubagentRoutePolicy(ctx);
        this.#baseProblems = [...result.problems];
        this.#applyFilter();
