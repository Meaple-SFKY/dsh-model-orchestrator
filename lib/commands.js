  ...
  if (assignmentCount > 0) {
    extra.push(
      `assignments: ${assignmentCount}` +
        ...
        (unassigned === 0 ? '' : `, ${unassigned} route(s) unassigned`),
    );
  }
  if (state?.researchedCount !== undefined) {
    extra.push(
      `researched: ${count(state.researchedCount)} route(s)` +
        (count(state.researchUnmatched) === 0 ? '' : `, ${count(state.researchUnmatched)} unconfirmed`),
    );
  }
