 ...
 */
export function scoreCapability(descriptor, profile) {
  let score = 0;
  let possible = 0;
  const matched = [];
  const hardFailures = [];
    ...
    switch (sig.type) {
      ...
      case 'keywords': {
        ...
        if (wantedTokens.length > 0) {
          hit = hits === 0 ? 0 : clamp(0.5 + (hits / wantedTokens.length) * 0.5, 0, 1);
          if (hits > 0) detail = `${hits}/${wantedTokens.length} declared cues`;
        }
        break;
      }
      default:
  ...
  const normalized = possible === 0 ? 0 : clamp(score / possible, 0, 1);
...
export function rankModels(pool, taxonomy, analysis, options = {}) {
  ...
  for (const profile of pool.models()) {
    ...
    for (const requirement of analysis.requirements) {
      ...
      const scored = scoreCapability(descriptor, profile);
