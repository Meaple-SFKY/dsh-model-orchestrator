  ...
  for (const sig of descriptor.signals) {
    ...
    switch (sig.type) {
      ...
      case 'reasoning': {
        ...
        //   `has`        — the provider exposes a LEVEL to pick (low/high/…)
        //   `reasons`    — the model reasons at all, which an automatic-reasoning
        //                  provider does while driving the depth itself
        // A requirement for reasoning (no levels named) is satisfied by either.
        // A requirement that NAMES levels needs the adjustable kind, because there
        // is nothing to select otherwise.
        const reasons = has || profile.facts?.reasoningMode === 'automatic';
        ...
        if (sig.max === 0) {
          ...
          if (sig.required === true) {
            hardFailures.push(`no reasoning is reported for ${profile.route}`);
            missingRequired = true;
          }
          ...
          detail = has
            ...
            : 'reasons automatically (the level is not selectable)';
        } else if (!has) {
          if (sig.required === true) {
            hardFailures.push(
              `none of the requested reasoning levels [${wanted.join(', ')}] is selectable for ${profile.route}`,
            );
            missingRequired = true;
          }
...
export function rankModels(pool, taxonomy, analysis, options = {}) {
  ...
  // Researched prices, normalised within this pool. Empty when the user has never
  // synced, in which case cost shaping falls back to the tier proxy.
  const priceRatios = isRecord(options.priceRatios) ? options.priceRatios : new Map();
  const priceRatio = (route) => {
    const value = priceRatios instanceof Map ? priceRatios.get(route) : priceRatios[route];
  ...
  for (const profile of pool.models()) {
    ...
    for (const requirement of analysis.requirements) {
      ...
      // Taken from the score, which already computed it for this profile: the
      // field existed and was recomputed here instead of being used.
      const satisfaction = clamp(scored.score * scored.confidence, 0, 1);
    ...
    //
    // A researched price, when the user has synced one, is a better basis than the
    // tier — a whole pool of `deep` routes made the tier proxy identical everywhere
    // and the preference did nothing at all. It is normalised against the most
    // expensive priced route IN THIS POOL, so "expensive" means expensive among
    // what this deployment can actually choose, and it still only reorders: the
    // hard requirements above rejected before any of this is consulted.
    let costPenalty = 0;
    if (preferCheaper) {
      const ratio = priceRatio(profile.route);
      if (ratio !== undefined) {
        // Same 0.08 ceiling the tier proxy uses, so enabling research cannot change
        // how much cost is allowed to matter — only what it is measured from.
        costPenalty = 0.08 * ratio;
      } else if (profile.derived?.tier === 'deep') costPenalty = 0.08;
