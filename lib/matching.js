 ...
 */
const DEEP_CUES = Object.freeze([
  'prove', 'proof', 'theorem', 'derive', 'derivation', 'root cause', 'debug',
  'verify', 'verified', 'audit', 'review', 'assess', 'evaluate', 'critique',
  'design', 'architect', 'trade-off', 'tradeoff', 'optimis', 'optimiz',
  'analyze', 'analyse', 'diagnose', 'reconcile', 'investigate', 'compare',
  'derive', 'formal', 'rigorous', 'careful',
  '证明', '推导', '定理', '根因', '调试', '审查', '评估', '设计', '架构', '优化', '分析', '排查',
]);

/**
 * Routine cues: mechanical, fully specified work with no hidden decisions.
 */
const ROUTINE_CUES = Object.freeze([
  'rename', 'format', 'reformat', 'typo', 'lint', 'boilerplate', 'list all',
  'extract', 'convert', 'translate', 'summarize', 'summarise', 'count',
  '重命名', '格式化', '错别字', '提取', '转换', '翻译', '摘要', '统计',
]);
...
export function analysisFromModel(taxonomy, input) {
  ...
  const order = ['trivial', 'simple', 'specialist', 'complex'];
  const complexity =
    claimed === undefined ? observed : order[Math.max(order.indexOf(claimed), order.indexOf(observed))];
 ...
 * @returns {TaskAnalysis}
 */
export function analysisFromText(taxonomy, text) {
  ...
  if (cueHits(haystack, DEEP_CUES) >= 1) {
  ...
  } else if (cueHits(haystack, ROUTINE_CUES) >= 1 && requirements.length <= 1) {
    requirements.push({
  ...
  if (cueHits(haystack, IMAGE_CUES) >= 1 && !requirements.some((req) => req.needsImageInput === true)) {
    requirements.unshift({
  ...
  return {
    summary: text,
    complexity: inferComplexity(requirements, text),
    requirements,
    source: 'local',
...
/** Cues that a task carries visual material the model must read. */
const IMAGE_CUES = [
  'image', 'images', 'screenshot', 'screenshots', 'picture', 'photo', 'diagram',
  'figure', 'attached image', 'the attachment', 'visual', 'render',
  '图片', '图像', '截图', '附图', '附件图', '照片', '示意图', '看图',
];
 ...
 */
export function inferComplexity(requirements, text = '') {
  const haystack = String(text ?? '').toLowerCase();
  const required = requirements.filter((req) => req.required === true);
  const strong = requirements.filter((req) => req.weight >= 0.5);

  const trivialCue = TRIVIAL_CUES.some((cue) => haystack.includes(cue));
  const multiStep = MULTI_STEP_CUES.filter((cue) => haystack.includes(cue)).length;
  const parallelCue = PARALLEL_CUES.some((cue) => haystack.includes(cue));
  const deep = cueHits(haystack, DEEP_CUES);
...
export function rankModels(pool, taxonomy, analysis, options = {}) {
  ...
  if (declaredPreferences.length > 0) {
    ...
    const rest = candidates.filter((candidate) => !chosen.includes(candidate));
    ordered = [...chosen, ...rest];
