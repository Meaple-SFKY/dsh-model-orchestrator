...
export class Orchestrator {
  ...
  #poolIsStale() {
    ...
    if (Array.isArray(advertised) && advertised.length > 0) {
      ...
      if (live !== undefined && live.join('\u0000') !== [...advertised].sort().join('\u0000')) {
        return true;
      }
    }
  ...
  #composeUnits(task, analysis, ranked, tier, suppliedUnits, assignments, prices, chain) {
    ...
    if (Array.isArray(suppliedUnits) && suppliedUnits.length > 0) {
      for (const [index, unit] of suppliedUnits.entries()) {
        ...
        // would be. A named route that is NOT in the live pool takes this path too — the
        // pool changes under a session (a deployment that edits its provider's models is
        // the case this was reported from), and losing the whole unit to a stale name is
        // worse than routing it and saying so.
        const decision = chooseRouteFor(
          [{ capability: capabilityId, weight: 1, required: false }],
          preferenceFor({ capabilityId, group: capabilityId.split('.')[0] }),
        );
        if (decision.chosen === undefined) {
          units.push({
            ...base,
        ...
        units.push({
          ...
          routeReason:
            requested === undefined
              ? `supplied by the caller, routed by "${capabilityId}": ${decision.chosen.matched.slice(0, 3).join('; ') || 'best available capability fit'}`
              : `supplied by the caller, but "${requested}" is not in the live pool; routed by "${capabilityId}" instead`,
      ...
      const waveResults = await mapLimited(wave, context.concurrency, async (unit) => {
        ...
        try {
          ...
          return {
            ...
            // A route the caller asked for that the pool did not have: reported so the
            // substitution is visible, rather than silently dispatching somewhere else.
            ...(str(unit.routeRequested) === undefined ? {} : { routeRequested: unit.routeRequested }),
            dependsOn: unit.dependsOn ?? [],
