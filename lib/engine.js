...
export class Orchestrator {
  ...
  #analyze(task, request) {
    ...
    const local = analysisFromText(this.taxonomy, task);
  ...
  #composeUnits(task, analysis, ranked, tier, suppliedUnits) {
    ...
    if (Array.isArray(suppliedUnits) && suppliedUnits.length > 0) {
      ...
      // Multi-agent plans execute in weight order, each stage seeing the prior
      // stage's findings. This is the smallest useful DAG: one wave per
      // specialist cluster, with later waves depending on earlier ones.
      if (tier === 'multi-agent') {
