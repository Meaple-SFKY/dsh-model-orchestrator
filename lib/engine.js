...
export class Orchestrator {
  ...
  #localAnalysis(task) {
    ...
    if (guided.length === 0) return local;
    const seeded = guided
  ...
  #resolveEffort(requested, route, fallback) {
    ...
    if (configured !== undefined && this.#advertises(route, configured)) return configured;
    return str(fallback);
  }
    ...
    if (Array.isArray(suppliedUnits) && suppliedUnits.length > 0) {
      ...
      for (const [group, requirements] of clusters) {
        ...
        const decision = chooseRouteFor(
          ...
          preferenceFor({ capabilityId: primary.capability, group }),
        );
        ...
        units.push({
          ...
          routeReason: (() => {
            ...
            if (decided.length > 0) return [...decided, ...decision.chosen.matched.slice(0, 2)].join('; ');
            if (decision.chosen.matched.length > 0) return decision.chosen.matched.slice(0, 4).join('; ');
            if (decision.chosen.pinned === true) return 'pinned by capability mapping';
            return 'best available capability fit';
          })(),
