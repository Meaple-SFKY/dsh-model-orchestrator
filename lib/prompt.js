...
export function usageSectionText(options) {
  ...
  const modeLine =
    mode === 'guided'
      ? 'Mode: Guides the user selected the capability areas for this session; honour them when matching, and prefer routes that serve the selected areas.'
...
- \`orchestrate_configure\` changes user preferences (Auto/Guided, cost, parallelism).
...
3. When you delegate, synthesize the returned specialist findings into one coherent
   answer. Resolve conflicts between specialists rather than concatenating them.
4. If a specialist result is missing, failed, or contradicts the others, close that gap
   yourself or retry it — do not forward a failed unit to the user as the answer.
5. If the task is in an area the capability vocabulary does not cover, say so and pass
   your own capability requirements to the orchestrator; it can form new ones.
6. Report routing only when it matters to the user (for example, which strengths were
