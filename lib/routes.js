  ...
  return {
    ...
    pool: {
      ...
      models: pool.models().map((model) => {
        ...
        // hiding it.
        // A route that reasons without advertising levels CAN be given one by hand
        // (see `effortPatch`), and the engine sends it. Reporting that as "ignored"
        // contradicted the dispatch — the panel said one thing, the child did
        // another — and made the client's `manual` branch unreachable.
        const acceptsEffort =
          efforts.length > 0
            ? efforts.includes(stored)
            : model.facts?.reasoningMode === 'automatic';
        const effective = stored !== undefined && acceptsEffort ? stored : undefined;
        return {
          route: model.route,
    ...
    // What this plugin actually turned on, and what it could not: a missing optional
    // dependency, a subsystem that failed to register, a surface this deployment does
    // not provide. Previously these were log lines, so a whole missing surface looked
    // like a plugin that simply did nothing.
    health: deps.health?.snapshot?.() ?? {
      missingDependencies: Array.isArray(deps.host?.optionalMissing) ? [...deps.host.optionalMissing] : [],
      subsystems: [],
      degraded: [],
      disabled: [],
    },
 ...
 * `webCtx.webServer`, and each route's own disposer is collected here and returned
 * as one function the caller runs at teardown. (It is deliberately NOT wrapped in
 * `webCtx.effect`: the service wait has to own that scope, and confining the
 * injection to an effect is what stopped the routes mounting at all — see §1.10.)
 *
...
function installRoutesOn(ctx, server, deps) {
  if (server === undefined || typeof server.register !== 'function') {
    deps.health?.disable('controlPanel', 'this deployment mounted no web server');
    deps.logger?.info?.(
      `${deps.host.name}: no web server is mounted, so the control panel routes were not registered (the tools work regardless).`,
    );
    return () => {};
  }
  deps.health?.enabled('controlPanel');

  const disposers = [];

  /**
   * When each session was first observed claiming to be running.
   *
   * Held across polls so a durable record that has been claiming `running` since long
   * before this board was opened can be told apart from a run that just started — the
   * difference between "still working" and "the host no longer runs this".
   */
  ...
  register(`${ROUTE_PREFIX}/state`, async (req, res) => {
    ...
    } else if (deps.engine?.poolTopologyChanged?.() === true) {
      // The provider set changed since the last discovery — most often because more
      // adapters registered after this plugin activated. Without this the panel kept
      // serving the activation-time pool (one provider) while every tool, whose call
      // goes through the engine's own staleness check, saw the real five. The check
      // it just ran is a synchronous registry read, so a poll stays cheap: a real
      // re-discovery happens only when the provider set actually changed.
      try {
        await deps.engine.refresh({});
      } catch (error) {
        deps.logger?.warn?.(
          `${deps.host.name}: could not follow a changed provider topology: ${String(error)}`,
        );
      }
    }
  ...
  register(`${ROUTE_PREFIX}/configure`, async (req, res) => {
    ...
    const result = applyPreferencePatch(deps.store, body, deps.pool, deps.taxonomy);
    // An allow/deny change re-narrows the pool, and re-applying the filter costs
    // nothing — so do it now, rather than leaving the panel showing the previous
    // sets while routing already used the new ones.
    let poolSize;
    try {
      poolSize = deps.engine?.refilterPool?.();
    } catch (error) {
      deps.logger?.warn?.(`${deps.host.name}: could not re-filter the pool: ${String(error)}`);
    }
    sendJson(res, 200, {
      ok: true,
      ...result,
      ...(Number.isSafeInteger(poolSize) ? { poolSize } : {}),
      state: stateDocument(deps),
    });
  });
  ...
  register(`${ROUTE_PREFIX}/sync`, async (req, res) => {
    ...
    // `start` builds its target list before it returns a promise, so it can throw
    // synchronously — a pool that is not ready, say. That must be a status, not a
    // rejected route handler the server has to absorb.
    try {
      // Not awaited: the sweep continues in the background and reports through
      // `/state`. The runner turns its own asynchronous failure into an error
      // status, so this cannot become an unhandled rejection either.
      void runner.start({ force: body?.force === true });
    } catch (error) {
      sendJson(res, 200, {
        ok: false,
        status: { status: 'error', error: String(error?.message ?? error) },
      });
      return;
    }
  ...
  register(`${ROUTE_PREFIX}/tree`, async (req, res) => {
    ...
    // What the plugin itself knows about these sessions outranks the durable record.
    const knownStatuses = new Map();
    try {
      const runs = deps.journal?.view?.(sessionId)?.runs ?? [];
      for (const run of runs) {
        for (const unit of run.units ?? []) {
          if (unit.childId === undefined) continue;
          if (unit.status === 'running' || unit.status === 'completed' || unit.status === 'failed') {
            knownStatuses.set(unit.childId, unit.status);
          }
        }
      }
    } catch (error) {
      deps.logger?.warn?.(`${deps.host.name}: could not read the run journal: ${String(error)}`);
    }
    const tree = buildAgentTree(sessionId, entries, { knownStatuses, seenRunning });
    // The harness tree is the REAL topology — one node per durable session, one edge
    // per reported parent. The journal is what the harness never knew: which capability
    // a unit covered, which route it ran on, what it produced, who reviewed it, and the
    // dependency and question edges between units. Joined on `childId`, the two make the
    // board's graph; neither alone can.
    // The root AND every descendant it enumerates: a unit that orchestrates its own
    // sub-run is part of this board, not a separate picture.
    const sessionIds = [sessionId, ...entries.map((entry) => str(entry?.id)).filter((id) => id !== undefined)];
    const journal = deps.journal?.view?.(sessionIds) ?? { runs: [] };
    sendJson(res, 200, { ok: true, ...tree, rows: flattenAgentTree(tree.nodes), journal });
  });

  /**
   * The language the browser half is actually showing.
   *
   * The harness resolves a language as "explicit host setting -> browser detection ->
   * en", and the browser-derived value is never written back to the host. So without
   * this report the host cannot know the UI language at all in the default case, and
   * every host-rendered string — a slash command's palette entry, its replies — stayed
   * English inside a Chinese UI. The client knows the answer, so it tells us. An
   * explicit user setting still outranks it.
   */
  register(`${ROUTE_PREFIX}/locale`, async (req, res) => {
    if (req.method !== 'POST') {
      res.writeHead(405, { allow: 'POST', 'cache-control': 'no-store' });
      res.end();
      return;
    ...
    if (language === undefined) {
      sendJson(res, 400, { error: 'language is required' });
      return;
    }
    // A failure here is not the caller's problem: the report is an optimisation, and
    // losing it only means the fallback language stands.
    const changed = deps.locale?.report?.(language) ?? false;
    sendJson(res, 200, {
      ok: true,
      language: deps.locale?.language ?? language,
      changed,
      source: deps.locale?.source,
    });
  });
