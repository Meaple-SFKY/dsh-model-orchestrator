 ...
 */
import { assertCompatible, missingOptionalServices } from './compatibility.js';
...
import { createHostLocale } from './host-locale.js';
...
export async function apply(ctx) {
  ...
  // Everything below is registered through this guard. It is what keeps one broken
  // subsystem from failing the composition entry — and with it the whole harness boot,
  // which disposes every unrelated plugin when an entry does not activate.
  const health = createHealth({
    host,
    logger,
    // The activation-time list is a snapshot; the health report is read on every poll.
    probe: () => missingOptionalServices(ctx),
  });

  // The host half needs the UI language for its OWN user-facing copy: the harness renders
  // a third-party command's description and hint verbatim and the command's replies are
  // plain text, so nothing translates them for us. Read from the durable `locale` setting
  // or from what the panel reports, never written.
  //
  // Declared HERE, before everything that consumes it, and that placement is load
  // bearing: the route installer's `ctx.inject` fires synchronously when the web server
  // is already mounted, and passing a `const` from below its declaration threw
  // `Cannot access 'locale' before initialization` — which the health guard then
  // correctly reported as "control panel disabled". `test/activation.test.js` drives
  // exactly that synchronous path, and caught the same mistake being made again when
  // the tools registry started taking the locale too.
  const locale = createHostLocale({ ctx, logger });
  ...
  // Every unit's complete answer is written to disk, so a long run stays readable
  // even when the harness bounds the tool result it carries inline. The writer sits
  // inside the calling session's working directory when one is known, and inside the
  // plugin's own state directory otherwise; either way it never fails a run.
  const artifacts = createArtifactWriter({
    logger,
    fallbackDir: directory,
  });
  const engine = new Orchestrator({ ctx, pool, taxonomy, store, logger, artifacts });
  ...
  health.run('poolFollowsAdapters', () =>
    ctx.effect(
      () =>
  ...
  // ---- tools --------------------------------------------------------------
  const tools = health.run('tools', () =>
    registerOrchestratorTools(ctx, {
      engine,
      pool,
      taxonomy,
      store,
  ...
  if (tools !== undefined) {
    ...
    // The section names the tools, so publishing it without them would describe a
    // surface that does not exist.
    health.disable('promptSection', 'the tools it describes were not registered');
  }
  ...
  const sync = health.run('sync', () =>
    createSyncRunner({
      ...
      runResearch: createWebResearch({
        ...
        logger,
        chooseRoute,
        // The web service is resolved lazily and may be absent for good: a headless
        // deployment has no search provider, and Sync has to report that rather than
        // pretend it can research.
        web: () => webService,
      }),
    }),
  );

  // Whether the deployment offers web search decides whether Sync can work at all, and
  // the service is frequently mounted AFTER this plugin activates — so the health record
  // asks on every read instead of deciding once, which would report a working surface as
  // permanently disabled on any deployment where it mounted late.
  health.check(
    'webSearch',
    () => webService !== undefined,
    'this deployment exposes no web search service',
  );
  ...
  const disposeRoutes =
    health.run('controlPanel', () =>
      installControlRoutesDeferred(ctx, {
        engine,
        pool,
        taxonomy,
        store,
        ...
        // Handed over so the panel can tell the host which language it is rendering —
        // the one source of truth the host cannot reach by itself.
        locale,
