      ...
      execute: async (args, exec) => {
        ...
        return toJsonSafe({
          ...
          models: pool.models().map((model) => ({
            ...
            ...(store.snapshot().preferences.reasoningEffort?.[model.route] === undefined
              ? {}
              : { reasoningEffort: store.snapshot().preferences.reasoningEffort[model.route] }),
          })),
  ...
  add(
    defineTool({
      ...
      execute: async (args) => {
        ...
        // Validated against the live pool, through the same rules the panel's
        // control route uses: a level a route does not advertise, or a route that
        // is not live, is reported instead of stored as an inert preference.
        const effort = effortPatch(args?.reasoningEffort, pool);
        const assignment = assignmentPatch(args?.capabilityAssignments, {
          taxonomy,
          models: pool.models(),
        });

        if (Object.keys(changes).length > 0 || effort.patch.length > 0 || Object.keys(assignment.patch).length > 0) {
          store.update((state) => {
        ...
        return toJsonSafe({
          ...
          applied: [
            ...Object.keys(changes),
            ...(effort.patch.length > 0 ? ['reasoningEffort'] : []),
            ...(Object.keys(assignment.patch).length > 0 ? ['capabilityAssignments'] : []),
          ],
          ...(unknown.length === 0 ? {} : { unknownCapabilities: unknown }),
          ...(effort.rejected.length === 0 && assignment.rejected.length === 0
            ? {}
            : { rejected: [...effort.rejected, ...assignment.rejected] }),
          // Reported at the moment they are written: an assignment naming a model
          // that is not in the pool right now is legal (the pool churns) but must
          // be visible.
