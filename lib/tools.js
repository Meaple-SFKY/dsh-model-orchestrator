...
function analysisFromArguments(args, parameters) {
  ...
  const unusedArguments = Object.keys(isRecord(args) ? args : {}).filter(
    (key) =>
      !declared.has(key) && args[key] !== undefined && !foldedIntoAnalysis.includes(key),
  );
    ...
    defineTool({
      ...
      parameters: {
        ...
        units: {
          type: 'array',
          description:
            'A caller-supplied unit graph, which replaces the one the orchestrator would build. Each entry is { id, capabilityId?, prompt, route? } and may set dependsOn to order stages. A `route` pins that unit; omit it and the unit is routed by the capability it names. Supplying units is an explicit request to delegate, so it overrides a tier the plugin merely inferred. Keep each `prompt` short: the run\'s `task` is added to every unit\'s prompt automatically, so repeating it here only makes this argument large enough to be written incorrectly.',
          items: {
            type: 'object',
            ...
            properties: {
              ...
              route: { type: 'string', description: 'Exact provider/model, or omit to let the orchestrator route it.' },
              dependsOn: {
                type: 'array',
                items: { type: 'string' },
