  ...
  add(
    defineTool({
      ...
      execute: async (args) => {
        ...
        if (Object.keys(changes).length > 0 || effort.patch.length > 0) {
          store.update((state) => {
        ...
        return toJsonSafe({
          ok: true,
          applied: [...Object.keys(changes), ...(effort.patch.length > 0 ? ['reasoningEffort'] : [])],
          ...(unknown.length === 0 ? {} : { unknownCapabilities: unknown }),
          ...(effort.rejected.length === 0 ? {} : { rejected: effort.rejected }),
          state: {
            mode: state.mode,
            guidedCapabilities: state.guided.capabilities,
            preferences: state.preferences,
