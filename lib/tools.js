    ...
    defineTool({
      ...
      parameters: {
        ...
        units: {
          type: 'array',
          description:
            'A caller-supplied unit graph, which replaces the one the orchestrator would build. Each entry is { id, capabilityId?, prompt, route? } and may set dependsOn to order stages. Supplying units is an explicit request to delegate, so it overrides a tier the plugin merely inferred.',
          items: {
            type: 'object',
