...
export function registerOrchestratorTools(ctx, deps) {
  ...
  add(
    defineTool({
      name: TOOL_NAMES.run,
      description:
        'Execute a task through the orchestrator: analyze it, match capabilities to live models, delegate each unit to a specialist subagent running on the matched route, and return every specialist result to you. You remain the owner of the final answer and must synthesize the results yourself. Use this for any task that is non-trivial, needs a specific strength, or spans more than one kind of expertise. Simple tasks should be executed directly instead.',
      parameters: {
