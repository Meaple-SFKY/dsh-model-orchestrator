...
import { candidateKeys, identityKey } from './model-identity.js';
...
/**
 * The shape a research answer is validated against.
 *
 * It is the contract `researchPrompt` asks the reconciling model for and
 * `normalizeResearch` enforces. It is written in the host's tool-parameter DSL
 * because that is the subset this project already compiles against the real host —
 * a test compiles it through `defineTool` — which keeps the declared shape honest
 * even though the sync path itself asks for JSON in a prompt rather than handing a
 * schema to a tool.
 *
...
export function normalizeResearch(input, { models = [], at = Date.now(), agentModel } = {}) {
  ...
  for (const row of raw) {
    ...
    entries[route] = {
      ...
      // The schema asks for USD only, so this is the unit of the numbers above
      // rather than something the answer supplies. Reading `row.currency` here
      // looked like it honoured the researcher and never could.
      ...(input1 === undefined && output1 === undefined ? {} : { currency: 'USD' }),
 ...
 *
 * Keyed by the PUBLIC identity — the normalised published name — whenever the
 * researcher confirmed one. That is the identity two routes actually share: the
 * same public model served by two providers is one model, and keying by the
 * route's own id string would store it twice and let the two copies drift.
 * Verified by lookup rather than by construction: `researchFor` matches any of a
 * live route's candidate keys against the map, and the name-derived key is one of
 * them, so a public-identity key is still found from the route.
 *
 * An unconfirmed route has no public identity to share, so it keeps its own key —
 * its "I could not confirm this" note belongs to that route and must not be
 * overwritten by another route's finding.
 *
...
export function mergeResearch(current, entries) {
  ...
  for (const entry of Object.values(entries ?? {})) {
    const publicKey = entry.matched === true ? identityKey(entry.publicName ?? '') : '';
    const key =
      publicKey !== ''
        ? publicKey
        : (entry.identityKeys?.find((candidate) => candidate !== '') ?? entry.route);
